...
(function () {
  ...
  const WSTART = {
    '1A': [0, 0], '3A': [2, 1], '5A': [6, 3], '7A': [9, 0],
    '8A': [11, 9], '9A': [12, 0], '12A': [16, 3],
    '2D': [0, 1], '4D': [2, 3], '6D': [8, 9], '9D': [12, 0],
    '10D': [14, 6], '11D': [15, 11], '12D': [16, 3],
  };

  const ACTIVE = [
    [0, 0, '1'], [0, 1, '2'], [0, 2, ''], [0, 3, ''], [0, 4, ''], [0, 5, ''],
    [1, 1, ''],
    [2, 1, '3'], [2, 2, ''], [2, 3, '4'], [2, 4, ''], [2, 5, ''],
    [3, 1, ''], [3, 3, ''],
    [4, 1, ''], [4, 3, ''],
    [5, 3, ''],
    [6, 3, '5'], [6, 4, ''], [6, 5, ''], [6, 6, ''], [6, 7, ''], [6, 8, ''], [6, 9, ''],
    [7, 3, ''],
    [8, 3, ''], [8, 9, '6'],
    [9, 0, '7'], [9, 1, ''], [9, 2, ''], [9, 3, ''], [9, 4, ''], [9, 5, ''], [9, 6, ''], [9, 7, ''], [9, 8, ''], [9, 9, ''], [9, 10, ''], [9, 11, ''], [9, 12, ''], [9, 13, ''],
    [10, 9, ''],
    [11, 9, '8'], [11, 10, ''], [11, 11, ''], [11, 12, ''], [11, 13, ''], [11, 14, ''], [11, 15, ''], [11, 16, ''],
    [12, 0, '9'], [12, 1, ''], [12, 2, ''], [12, 3, ''], [12, 4, ''], [12, 5, ''], [12, 6, ''], [12, 7, ''], [12, 8, ''], [12, 9, ''],
    [13, 0, ''], [13, 9, ''],
    [14, 0, ''], [14, 6, '10'], [14, 9, ''],
    [15, 0, ''], [15, 6, ''], [15, 9, ''], [15, 11, '11'],
    [16, 0, ''], [16, 3, '12'], [16, 4, ''], [16, 5, ''], [16, 6, ''], [16, 7, ''], [16, 8, ''], [16, 9, ''], [16, 10, ''], [16, 11, ''], [16, 12, ''], [16, 13, ''],
    [17, 0, ''], [17, 3, ''], [17, 6, ''], [17, 11, ''],
    [18, 3, ''], [18, 6, ''], [18, 11, ''],
    [19, 3, ''], [19, 6, ''], [19, 11, ''],
    [20, 3, ''], [20, 6, ''], [20, 11, ''],
    [21, 3, ''],
  ...
  function buildGrid() {
    ...
    for (let r = 0; r < ROWS; r++) {
      ...
      for (let c = 0; c < COLS; c++) {
        ...
        td.style.cssText = 'width:28px;height:28px;padding:0;position:relative;'
          + (active
        ...
        if (active) {
          if (cellNum[key]) {
            ...
            s.style.cssText = 'position:absolute;top:1px;left:2px;font-size:8px;line-height:1;pointer-events:none;color:var(--text-color);';
            td.appendChild(s);
          }
          ...
          inp.dataset.r = r;
          inp.dataset.c = c;
          inp.style.cssText = 'position:absolute;inset:0;width:100%;height:100%;border:none;background:transparent;'
  ...
  function inp(r, c) { return document.querySelector('#cw-grid input[data-r="' + r + '"][data-c="' + c + '"]'); }
  ...
  function onInput(e) {
    ...
    e.target.value = v;
    if (v) userGrid[r + ',' + c] = v; else delete userGrid[r + ',' + c];
    e.target.style.color = 'var(--text-color)';
    if (v && selected) {
      const { dir } = selected;
      const nr = r + (dir === 'D' ? 1 : 0), nc = c + (dir === 'A' ? 1 : 0);
      if (cellActive[nr + ',' + nc]) { selected = { r: nr, c: nc, dir }; inp(nr, nc)?.focus(); highlight(); }
    }
  }
  ...
  function move(r, c, dr, dc) {
    ...
    if (cellActive[nr + ',' + nc]) {
      ...
      inp(nr, nc)?.focus(); highlight(); showClue();
    }
  }
  ...
  function getWord(r, c, dir) {
    for (const wk of Object.keys(WSTART)) {
      ...
      for (let i = 0; i < len; i++) {
        const cr = wr + (dir === 'D' ? i : 0), cc = wc + (dir === 'A' ? i : 0);
  ...
  function wordCells(wk) {
    const [wr, wc] = WSTART[wk], dir = wk.slice(-1);
    return SOL[wk].map((_, i) => ({ r: wr + (dir === 'D' ? i : 0), c: wc + (dir === 'A' ? i : 0) }));
  }

  function highlight() {
    document.querySelectorAll('#cw-grid td').forEach(td => {
      if (cellActive[(+td.children[td.children.length - 1]?.dataset?.r || '') + ',' + (+td.children[td.children.length - 1]?.dataset?.c || '')] || true)
    ...
    if (wk) wordCells(wk).forEach(({ r, c }) => {
      const el = inp(r, c); if (el) el.parentElement.style.background = 'rgba(128,128,255,0.18)';
    });
  ...
  function buildClues() {
    const ao = document.getElementById('cw-across'), dw = document.getElementById('cw-down');
    ao.innerHTML = ''; dw.innerHTML = '';
    CLUES_A.forEach(({ n, text }) => {
      const li = document.createElement('li'); li.value = n; li.textContent = text;
  ...
  function jumpTo(wk) {
    ...
    inp(r, c)?.focus(); highlight(); showClue();
  }

  function showClue() {
    const el = document.getElementById('cw-clue-active');
    if (!selected) { el.textContent = 'Cliquez une case'; return; }
    const wk = getWord(selected.r, selected.c, selected.dir);
    if (!wk) { el.textContent = ''; return; }
    const n = parseInt(wk), dir = wk.slice(-1);
  ...
  window.cwVerify = function () {
    ...
    let filledCount = 0;
    filled.forEach(el => { if (el.value) filledCount++; });
    if (filledCount === 0) { msg.textContent = ''; return; }
    if (ok === total) { msg.textContent = '✓ Parfait !'; msg.style.color = '#27ae60'; }
    else { msg.textContent = ok + '/' + total + ' corrects'; msg.style.color = '#e67e22'; }
  };
  ...
  window.cwReveal = function () {
    document.querySelectorAll('#cw-grid input').forEach(el => {
      const key = el.dataset.r + ',' + el.dataset.c;
      const exp = EXPECTED[key];
      if (exp) { el.value = exp; userGrid[key] = exp; el.style.color = '#2980b9'; }
    });
